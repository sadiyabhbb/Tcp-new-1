...
async def TcPOnLine(ip, port, key, iv, AutHToKen, reconnect_delay=0.5):
    global online_writer , spam_room , whisper_writer , spammer_uid , spam_chat_id , spam_uid , XX , uid , Spy,data2, Chat_a
    while True:
        try:
...
async def TcPChaT(ip, port, AutHToKen, key, iv, LoGinDaTaUncRypTinG, ready_event, region , reconnect_delay=0.5):
    ...
    global spam_room , whisper_writer , spammer_uid , spam_chat_id , spam_uid , online_writer , chat_id , XX , uid , Spy,data2, Chat_a
    while True:
        try:
            reader , writer = await asyncio.open_connection(ip, int(port))
                    ...
                    if response:
                        ...
                        if inPuTMsG.strip().startswith('@a'):
                            ...
                            try:
                                ...
                                message = f'[B][C]{get_random_color()}\nACITVE Emote {get_random_color()} ALAMIN GAMEING-> {get_random_color()}{xMsGFixinG(uid)}\n'
...
async def MaiiiinE():
    Uid , Pw = '4222106409','1E8AE66AC23B4ADE528BF44D05B118167CA1BA6685EAE262C51499ED479CDE5D'
